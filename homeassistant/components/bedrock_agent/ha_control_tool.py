...
class HAToolRegistry:
    ...
    def __init__(self) -> None:
        ...
        self.tools_by_name: dict[str, tuple[Tool, str]] = {}  # tool_name -> (tool, api_id)
        ...
        for api in apis:
            try:
                ...
                for ha_tool in api_instance.tools:
                    ...
                    _LOGGER.debug(
                        "Registered HA tool: %s from API: %s",
                        ha_tool.name,
                        api.id
                    )
                    
                _LOGGER.debug(
                    "Loaded API: %s with %d tools",
                    api.id,
                    len(api_instance.tools)
                )
    ...
    def get_tool_descriptions(self) -> str:
        ...
        for tool_name, (tool, api_id) in sorted(self.tools_by_name.items()):
    ...
    async def async_call_tool(
        ...
        try:
            ...
            _LOGGER.exception("Error calling HA tool %s: %s", tool_name, err)
            ...
            if "Failed to call turn_on" in error_msg and "scene." in error_msg:
                ...
                return {
                    "error": f"Scenes cannot be activated with HassTurnOn. Try using the scene name as tool_name directly, or check available tools with GetLiveContext."
                }
